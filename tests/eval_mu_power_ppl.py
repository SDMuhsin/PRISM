"""
PPL Evaluation: μ-Power Scaling

Key finding: α=0 (no μ in importance) shows huge MSE improvement on single layers.
Test if this translates to PPL improvement.

importance = |W| × ||X|| / μ^α

α = 0: Just magnitude × activation (no μ)
α = 1: Current baseline (divide by μ)
"""
...
class MuPowerQuantizedLinear(nn.Module):
    """Quantized linear with μ^alpha importance scaling."""

    def __init__(self, weight, bias, activations, sparsity, nbits, group_size, alpha=1.0):
        super().__init__()
        W = weight.data.float()
        K, N = W.shape
...
def replace_layers(model, activations, sparsity, nbits, group_size, alpha=1.0):
    replaced = 0
    for name, module in list(model.named_modules()):
        if isinstance(module, nn.Linear) and 'lm_head' not in name and name in activations:
            ...
            new_module = MuPowerQuantizedLinear(
...
def main():
    print("="*70)
    print("μ-POWER SCALING PPL EVALUATION")
    print("="*70)
    ...
    for alpha in [0.0, 0.5, 1.0]:
        ...
        replaced = replace_layers(model, activations, sparsity, nbits, group_size, alpha=alpha)
        print(f"Replaced {replaced} layers")
        ppl = eval_ppl(model, tokenizer, max_samples=32)
        print(f"α={alpha} PPL: {ppl:.2f}")
        results[alpha] = ppl
    ...
    for alpha in [0.0, 0.5, 1.0]:
        change = (baseline - results[alpha]) / baseline * 100
        marker = " (baseline)" if alpha == 1.0 else ""
        print(f"α={alpha}: {results[alpha]:.2f} ({change:+.2f}%){marker}")

"""
Evaluate Adaptive MWC at 3-bit, 35% sparsity (correct baseline config).

Baseline to beat: SINQ-Sparse 3-bit 35% sparsity = PPL 28.10 (original) / 18.15 (with inverse μ fix)
"""
...
def sparse_quantize_model(model, calibration_data, sparsity=0.35, nbits=3,
    ...
    print(f"\nSparse quantization: {nbits}-bit, {sparsity*100:.0f}% sparsity, compensation_mode={compensation_mode}")
...
def test_model(model_name, nbits=3, sparsity=0.35):
    """Test methods on a specific model."""
    print(f"\n{'='*70}")
    ...
    # Test 2: Standard OBS (SINQ-Sparse baseline)
    print("\n--- Standard OBS (SINQ-Sparse Baseline) ---")
    model = AutoModelForCausalLM.from_pretrained(
        model_name, torch_dtype=torch.float16, device_map=device
    )
    model = sparse_quantize_model(model, calibration_data, sparsity=sparsity, nbits=nbits,
                                   compensation_mode='fast', device=device)
    ppl_std_obs = evaluate_perplexity(model, test_data, device)
    print(f"Standard OBS PPL: {ppl_std_obs:.2f}")
    results['std_obs'] = ppl_std_obs
    del model
    torch.cuda.empty_cache()
    gc.collect()
    ...
    results['adaptive_mwc'] = ppl_adaptive_mwc
    del model
    torch.cuda.empty_cache()
    gc.collect()
...
def main():
    print("="*70)
    print("ADAPTIVE MWC EVALUATION - 3-BIT, 35% SPARSITY")
    print("="*70)
    print("\nBaseline to beat: SINQ-Sparse 3-bit 35% = PPL 18.15 (with inverse μ)")
    print("Original buggy baseline was PPL 28.10")
    ...
    print("="*70)
    print(f"\n{'Model':<10} | {'FP16':>8} | {'Std OBS':>10} | {'Adaptive MWC':>12} | {'vs Baseline':>12}")
    print("-"*65)

    for model_name, results in all_results.items():
        improvement = (results['std_obs'] - results['adaptive_mwc']) / results['std_obs'] * 100
        print(f"{model_name:<10} | {results['fp16']:>8.2f} | {results['std_obs']:>10.2f} | {results['adaptive_mwc']:>12.2f} | {improvement:>+11.2f}%")

    print("\n" + "="*70)
    print("VERDICT")
    print("="*70)
    for model_name, results in all_results.items():
        if results['adaptive_mwc'] < results['std_obs']:
            improvement = results['std_obs'] - results['adaptive_mwc']
            print(f"✓ {model_name}: Adaptive MWC beats Standard OBS by {improvement:.2f} PPL points")
        else:
            degradation = results['adaptive_mwc'] - results['std_obs']
            print(f"✗ {model_name}: Adaptive MWC worse than Standard OBS by {degradation:.2f} PPL points")

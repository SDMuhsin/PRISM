// CPR-SINQ: Heavy Register Blocking WMMA Kernel
//
// This kernel uses very aggressive register blocking to maximize
// Tensor Core utilization. Each warp computes a larger output tile.
...
// Block configuration - smaller block with fewer warps but more work per warp
constexpr int BLOCK_M = 128;
constexpr int BLOCK_N = 128;
constexpr int BLOCK_K = 64;  // Larger K for more compute per load

// Thread configuration - only 4 warps but each does more work
constexpr int WARP_SIZE = 32;
constexpr int NUM_WARPS = 4;
constexpr int NUM_THREADS = NUM_WARPS * WARP_SIZE;  // 128

// Warp layout: 2 warps in M, 2 warps in N
constexpr int WARPS_M = 2;
constexpr int WARPS_N = 2;

// Each warp computes 64x64 = 4x4 = 16 WMMA tiles
...
// =============================================================================
// Heavy Register Blocking Kernel
// =============================================================================

__global__ void wmma_matmul_heavy_kernel(
    ...
    half* smem_A = smem;
    half* smem_B = smem + BLOCK_M * SMEM_A_STRIDE;
    ...
    // Heavy register blocking: 4x4 = 16 accumulators per warp
    ...
    for (int k_tile = 0; k_tile < num_k_tiles; k_tile++) {
        ...
        // Compute - each warp processes 64x64 output
        const int warp_row_base = warp_m * (BLOCK_M / WARPS_M);  // 0 or 64
        const int warp_col_base = warp_n * (BLOCK_N / WARPS_N);  // 0 or 64
        ...
        for (int k = 0; k < BLOCK_K; k += WMMA_K) {
            ...
            for (int tm = 0; tm < WARP_TILES_M; tm++) {
                ...
                wmma::load_matrix_sync(a_frag[tm], smem_A + a_row * SMEM_A_STRIDE + k, SMEM_A_STRIDE);
            }

            // Load B fragments and compute all MMA operations
            #pragma unroll
            for (int tn = 0; tn < WARP_TILES_N; tn++) {
                wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half, wmma::row_major> b_frag;
                int b_col = warp_col_base + tn * WMMA_N;
                wmma::load_matrix_sync(b_frag, smem_B + k * SMEM_B_STRIDE + b_col, SMEM_B_STRIDE);

                // Compute all 4 M tiles with this B fragment
                #pragma unroll
                for (int tm = 0; tm < WARP_TILES_M; tm++) {
...
torch::Tensor wmma_matmul_heavy(
    ...
    int smem_A_size = BLOCK_M * SMEM_A_STRIDE * sizeof(half);
    int smem_B_size = BLOCK_K * SMEM_B_STRIDE * sizeof(half);
    int smem_size = smem_A_size + smem_B_size;

    dim3 grid((N + BLOCK_N - 1) / BLOCK_N, (M + BLOCK_M - 1) / BLOCK_M);
    dim3 block(NUM_THREADS);

    wmma_matmul_heavy_kernel<<<grid, block, smem_size, at::cuda::getCurrentCUDAStream()>>>(

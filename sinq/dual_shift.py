...
def quantize_rtn(
    ...
    group_size: int = None        # Group size for quantization (None = per-row)
):
    """
    Round-to-Nearest (RTN) quantization with optional group-wise scales.

    Args:
        matrix: Input tensor [K, N] to quantize
        min_max: [min_int, max_int] quantization range
        mode: "uniform" for standard RTN, "nf4"/"nf3" for non-uniform codebooks
        group_size: If set, use per-group quantization along dim=1 (columns).
                   This gives finer scale estimation and reduces quantization error.
                   Default None means per-row quantization.

    Returns:
        q: Quantized tensor
        scales: Scale tensor (shape depends on group_size)
        zeros: Zero-point tensor
        orig_dtype: Original input dtype
    """
    w = matrix
    orig_dtype = w.dtype
    dev = w.device
    ...
    if use_groups:
        # Pad if N not divisible by group_size
        if N % group_size != 0:
        ...
        # Reshape to [K * n_groups, group_size] for per-group quantization
        w_grouped = w.view(K, n_groups, group_size).permute(0, 1, 2).reshape(K * n_groups, group_size)
    else:
        w_grouped = w
        n_groups = 1
        pad_size = 0
    ...
    if mode.lower() in ("nf4", "nf3"):
        ...
        if not nf_use_shift:
            scales = w_grouped.abs().amax(dim=1, keepdim=True).clamp_min(1e-4)
            zeros  = torch.zeros_like(scales)
            norm   = w_grouped / scales
        else:
            if nf_shift_kind == "minmax":
                ...
                norm   = w_grouped / scales + zeros
            elif nf_shift_kind == "mean":
                mu     = w_grouped.mean(dim=1, keepdim=True)
                x      = w_grouped - mu
    ...
    # Use appropriate dtype based on bit precision to avoid overflow
    # int8 can only hold -128 to 127, but 8-bit quantization needs 0-255
    if max_int > 127:
        q = q.to(torch.int16)  # Use int16 for 8-bit quantization
    else:
        q = q.to(torch.int8)
...
def quantize_dual_scale_shift(matrix, min_max, method='sinq', awq_scale=None, group_size=64):
    ...
    Args:
        matrix: Weight matrix [K, N] to quantize
        min_max: [min_int, max_int] quantization range (e.g., [0, 7] for 3-bit)
        method: Quantization method ('sinq', 'sinq_noz', 'hqq', 'nf4', 'nf3', etc.)
    ...
    Returns:
        ...
        s1: Row scales (includes RTN scale and mu2)
        s2: Column scales (mu1)
        z: Zero points

    Dequantization formula: W = (q - z) * s1 * s2

    With group_size: s1 has shape [K, n_groups, 1], z has shape [K, n_groups, 1]
    Without group_size: s1 has shape [K, 1], z has shape [K, 1]
    """
    dtype = matrix.dtype
    dev = matrix.device
    K, N = matrix.shape
    ...
    if not ('hqq' in method):
        if 'noz' in method:
            ...
            if "nf4" in method.lower():
                q, scales, z, _ = quantize_rtn(matrix, min_max, mode="nf4", group_size=group_size)
            elif "nf3" in method.lower():
                q, scales, z, _ = quantize_rtn(matrix, min_max, mode="nf3", group_size=group_size)
            else:
                q, scales, z, _ = quantize_rtn(matrix, min_max, mode="uniform", group_size=group_size)
    ...
    # s2 = mu1 (column-wise scale)
    scales2 = torch.ones(1, N).to(dev).to(mu1.dtype) * mu1

    # s1 = scales * mu2 (row-wise scale, combined with RTN scale)
    # Handle both grouped and non-grouped cases
    if len(scales.shape) == 3:
        # Grouped: scales is [K, n_groups, 1], mu2 is [K, 1]
        scales = scales * mu2.unsqueeze(1)  # [K, n_groups, 1]
    else:
        # Non-grouped: scales is [K, 1], mu2 is [K, 1]
        scales = scales * mu2
